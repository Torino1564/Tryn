...
TestApp::TestApp(std::shared_ptr<win::IWindow> wnd_, std::shared_ptr<gfx::IGraphics> gfx_)
{
	...
	// Index Buffer
	auto dummyIB = gfx::BindablePool::Resolve<gfx::IIndexBuffer>(Gfx(), std::make_shared<std::vector<int>>(indices), "cube");
	...
	std::vector<std::variant<std::pair<std::string, gfx::VertexBuffer>, std::shared_ptr<gfx::IVertexBuffer>>> vertexBufferArr;
	...
	auto pPolyVB = Gfx().CreatePolyVertexBuffer(std::move(vertexBufferArr) , "coloredCube");
