...
#include "Core/src/scr/NodeRegister.h"
...
namespace tryn::ed
{
        ...
        : App(pWnd, pGfx), pContext(ned::CreateEditor())
    {
        ...
        scr::NodeRegister::Get().RegisterNodeType<scr::EntryNode>();
        scr::NodeRegister::Get().RegisterNodeType<scr::ConditionalNode>();
        scr::NodeRegister::Get().RegisterNodeType<scr::WaitNode>();
        scr::NodeRegister::Get().RegisterNodeType<scr::StateNode>();
        scr::NodeRegister::Get().RegisterNodeType<scr::SetVarNode>();
        gfx->SetVsyncFlag(true);

        submitBehaviour = [](scr::ScriptNode& node)
        {
	        if (!node.placed)
			{
			...
			for (auto& pinId : node.pinIds)
			{
				...
				if (pinInfo.kind == scr::PinKind::Input)
...
void ed::TrynEditorApp::LoadGraph()
{
    ...
    scr::ScriptGraph* pNewGraph = new scr::ScriptGraph;
    ...
    
	{
    	...
    	const auto pFunc = lib.get_function<bool(scr::TypeRegister*)>("GetRegister");
    	data.pRegister = std::make_unique<scr::TypeRegister>();
...
void ed::TrynEditorApp::RMBMenu(ImVec2 pos)
{
    ...
    if (ImGui::BeginPopup("Right Click Menu"))
    {
	    ...
	    {
	    	for (const auto& [uuid, nodeInfo] : scr::NodeRegister::Get().Map())
	    	{
