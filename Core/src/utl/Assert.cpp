...
namespace tryn::utl
{
	...
	Assertion::Assertion(std::string expression, const char* file, const char* function, int line, Consequence consequence, std::wstring msg)
		:
		file_{ utl::ToWide( file ).data()},
		function_{ utl::ToWide(function).data() },
		line_{ line },
		consequence_{ consequence },
		skip_depth_{ skip_depth }
	{
		stream_ << L"Assertion Failed! " << utl::ToWide(expression) << "\n" << msg << "\n";
	}
	Assertion::~Assertion()
	{
