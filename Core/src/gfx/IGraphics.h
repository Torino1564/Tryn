...
namespace tryn::gfx
{
	...
	public:
		...
		using SupportedBindables = std::tuple<
			...
			Register<class ITexture,
					std::tuple<const std::shared_ptr<class Texture>&, uint8_t, enum TextureUsage>,
					std::tuple<spa::DimensionsI, enum TextureFormat, uint8_t, TextureUsage>>,
		...
		std::shared_ptr<T> CreateBindable(Args&&... args) const
		{
			...
			{
				for (auto [index, rtti] : std::ranges::enumerate_view(bindableConstructorsRTTI[indexInTupleOfBindables]))
				{
		...
		Args&&... args) const
	{
		...
		if constexpr (N == 0 && std::tuple_size_v<ParameterTuplesTuple> == 0)
		{
			...
			auto pCastedFunc = std::any_cast<std::shared_ptr<Interface>(*)(const IGraphics&, std::tuple<Args...>)>(pFunc);
