...
namespace tryn::gfx
{
	...
	gfx(gfx)
	{
		...
		auto pTexture = TexturePool::Resolve(tex);
		this->pTextureResource = pTexture;
	...
	dx11::DX11Texture::DX11Texture(const Graphics& gfx, std::shared_ptr<Texture> pTexture, uint8_t slot)
		:
		gfx(gfx)
	{
		this->path = pTexture->GetPath();
		this->slot = slot;
