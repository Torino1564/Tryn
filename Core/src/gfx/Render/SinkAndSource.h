...
namespace tryn::gfx
{
	...
	public:
		...
		void Set(std::shared_ptr<T>& pResource, const std::string& exposureName)
		{
			...
			const auto it = std::ranges::find_if(exposures, [&](const auto& tuple)
				{
					return tuple.first == exposureName;
				});
			trynass(it == exposures.end());

			// Append new element
			exposures.emplace_back({
					exposureName,
					ZT_TYPE_UUID(T),
					pResource
				});
		}
		bool IsBounded(const std::string& exposureName) override
		{
	...
	private:
		...
		std::vector<std::tuple<std::string, utl::UUID_t, std::any>> exposures;
	};
